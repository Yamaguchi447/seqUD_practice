...
class SeqUD(BaseSampler):
    ...
    def _calculate_threshold(self):
        # 探索空間の体積を計算
        volume = np.prod([self.upper_bounds[k] - self.lower_bounds[k] for k in self.param_space])
        # 探索空間における点の密度を計算
    ...
    def _shrink_search_space(self):
        ...
        existing_points = self.current_design
        ...
        # 閾値を通過した点をbese_udに追加
        self.base_ud = np.vstack([self.base_ud, new_points])
    ...
    def before_trial(self, study: Study, trial: FrozenTrial) -> None:
        ...
        grid_id = trial.number % self.n_runs_per_stage

        if grid_id >= self.current_design.shape[0]:
            grid_id = self.current_design.shape[0] - 1
        trial.system_attrs["grid_id"] = grid_id
